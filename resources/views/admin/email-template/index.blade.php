            ...
            <div class="col-lg-12">
                <div class="row tabelhed d-flex justify-content-between">
                    <div class="col-lg-2 col-md-2 col-sm-2 d-flex">
                            <a class="ad-btn btn text-center" href="{{ route('admin.email-templates.create') }}"> Add</a>
                    </div>
                    ...
                    <div class="col-lg-10 col-md-10"> 
                        ...
                        <div class="right-item d-flex justify-content-end" >
                            ...
                            <form action="" method="GET" class="d-flex">
                                ...
                                <a href="{{ route('admin.email-templates.index') }}" class="btn-sm reload-btn">
                                ...
                                @if(count($data)>0)
                                    ...
                                    @foreach($data as $key => $value)
                                        <tr data-entry-id="{{ $value->id }}">
                                            <td>{{ $i++ ?? ''}}</td>
                                            <td>{{ $value->title ?? '' }}</td>
                                            <td>{{ $value->email_key ?? '' }}</td>
                                            <td>{{ $value->email_subject ?? '' }}</td>
                                            <td class="text-center">
                                                <span class="btn btn-xs {{ $value->status ? 'btn-success' : 'btn-danger' }} text-capitalize change-status" route="{{ route('admin.email-templates.change-status', $value->id) }}">{{ $value->status ? 'Active' : 'In-Active' }}</span>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('admin.email-templates.show', $value->id) }}" class="btn btn-sm btn-icon p-2">
                                                    <i class="mdi mdi-eye mx-1" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="View"></i>
                                                </a>
            
                                                <a href="{{ route('admin.email-templates.edit', $value->id) }}" class="btn btn-sm btn-icon p-2">
                                                    <i class="mdi mdi-pencil" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="Edit"></i>
                                                </a>

                                                <button type="submit" class="btn btn-sm btn-icon p-2 delete-record" route="{{ route('admin.email-templates.destroy', $value->id) }}"><i class="mdi mdi-delete" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="Delete"></i></button> 
